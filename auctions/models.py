from django.contrib.auth.models import AbstractUser
from django.db import models
from datetime import datetime


class User(AbstractUser): 
    pass


class listing(models.Model):
    title = models.CharField(max_length=64)
    description = models.TextField(blank=True,null=True)
    bid = models.FloatField()
    image = models.TextField(blank=False,null=False,default="data:image/jpeg;base64,")
    category = models.CharField(max_length=64)
    date = models.DateTimeField(default=datetime.now(), blank=False)
    by = models.CharField(max_length=64,blank=True)
    changed = models.TextField(null = False,default="False")
    active = models.TextField(null = False,default="True")

    def __str__(self):
        return f"{self.title} : {self.bid}"

class Bidding(models.Model):
    user = models.CharField(max_length=64,blank=True,null=True)
    listing_id = models.IntegerField(blank=True,null=True)
    bid = models.FloatField()
    date = models.DateTimeField(default=datetime.now(), blank=False)

    def __str__(self):
        return f"{self.user} : {self.bid}"

class Watchlist(models.Model):
    user = models.CharField(max_length=64,blank=True,null=True)
    listing_id = models.ForeignKey(listing,on_delete=models.CASCADE,related_name="rewatch")

    def __str__(self):
        return f"{self.user} : {self.listing_id}"

class comment(models.Model):
    user = models.CharField(max_length=64,blank=True,null=True)
    listing_id = models.IntegerField(blank=True,null=True)
    comments = models.TextField(blank=True,null=True)
    date = models.DateTimeField(default=datetime.now(), blank=False)
    edited = models.TextField(null = False,default="False")


    def __str__(self):
        return f"{self.user} : {self.listing_id} - {self.comments}"
